package com.open.lee.myhttpframework.entity;
...
public class BigMultipartEntity implements HttpEntity{
    ...
    private HashMap<String, File> mBigFileToBeSent = new HashMap<>();

    public BigMultipartEntity() {
    ...
    public void addBigFilePart(final String key, final File file){
        mBigFileToBeSent.put(key, file);
    }
    ...
    public void writeTo(final OutputStream outstream) throws IOException {
        ...
        //
        writeBigFileTo(outstream);
        outstream.write(mOutputStream.toByteArray());
    }

    public void writeBigFileTo(OutputStream outputStream) throws IOException {
        for (HashMap.Entry<String, File> entry : mBigFileToBeSent.entrySet()){
            String path = entry.getValue().getPath();
            InputStream fin = new FileInputStream(entry.getValue());
            Log.d("test", entry.getValue().getPath());
            outputStream.write(("--" + mBoundary + "\r\n").getBytes());
            final String type = CONTENT_TYPE + TYPE_OCTET_STREAM + NEW_LINE_STR;
            outputStream.write(getContentDispositionBytes(entry.getKey(), entry.getValue().getName()));
